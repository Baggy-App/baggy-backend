defmodule BaggyBackendWeb.ProductListControllerTest do
  ...
  describe "index" do
    test "lists all product_lists", %{conn: conn} do
      conn = get(conn, Routes.product_list_path(conn, :index))
  ...
  describe "create product_list" do
    test "renders product_list when data is valid", %{conn: conn} do
      conn = post(conn, Routes.product_list_path(conn, :create), product_list: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.product_list_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.product_list_path(conn, :create), product_list: @invalid_attrs)
  ...
  describe "update product_list" do
    ...
    test "renders product_list when data is valid", %{conn: conn, product_list: %ProductList{id: id} = product_list} do
      conn = put(conn, Routes.product_list_path(conn, :update, product_list), product_list: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, product_list: product_list} do
      conn = put(conn, Routes.product_list_path(conn, :update, product_list), product_list: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete product_list" do
    ...
    test "deletes chosen product_list", %{conn: conn, product_list: product_list} do
      conn = delete(conn, Routes.product_list_path(conn, :delete, product_list))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.product_list_path(conn, :show, product_list))
      end
    end
  end
